...
- `internal clipboard write`: a clipboard mutation caused by Steward itself while performing refine replacement or Capture output handling.
...
- Refine replacement in `replaceSelectedText(with:)` temporarily writes replacement text and later restores previous clipboard content.
- Capture flow in `copyTextToClipboard(_:)` writes extracted text to the clipboard.

### V1 Suppression Policy

Recommended policy for v1:

- suppress temporary clipboard writes used only to make Refine work,
- record the final Capture copy result because it is user-visible clipboard content.

That means:

- do not store the temporary selection copy generated by `Command+C` simulation,
- do not store the clipboard restoration step after refine replacement,
- do not store transient internal replacement text if it is only part of the Refine paste workflow,
- do store Capture text copied as the final result of Capture.
...
- Loading failure should not block core app features like Refine or Capture.
...
- The feature must not add noticeable latency to Refine, Capture, or menu interactions.
...
- Refine clipboard internals do not spam the history.
- Capture output is captured in history under the recommended v1 policy.
...
### Refine Flow Noise Suppression

- Trigger Refine on selected text.
- Verify temporary internal clipboard writes do not create extra history entries.

### Capture Flow

- Trigger screen Capture and let Steward copy the extracted text.
- Verify the final copied Capture text appears as a history record.
