...
struct SettingsView: View {
    ...
    @State private var openAIModelID: String = preferenceValue(
        forKey: "openAIModelID", defaultValue: OpenAIClient.defaultModelID)
    @State private var geminiAPIKey: String = UserDefaults.standard.string(forKey: "geminiAPIKey") ?? ""
    @State private var geminiModelID: String = preferenceValue(
        forKey: "geminiModelID", defaultValue: GeminiClient.defaultModelID)
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        TextField(OpenAIClient.defaultModelID, text: $openAIModelID)
                            ...
                            .onChange(of: openAIModelID) { newValue in
                                savePreferenceValue(
                                    newValue, forKey: "openAIModelID", defaultValue: OpenAIClient.defaultModelID)
                                NotificationCenter.default.post(name: .checkOpenAIStatus, object: nil)
                            }
                    }

                    Text(
                        "Your API key is needed to use the grammar check feature.\nModel ID defaults to \(OpenAIClient.defaultModelID) when left empty."
                    )
                    .font(.caption)
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        TextField(GeminiClient.defaultModelID, text: $geminiModelID)
                            ...
                            .onChange(of: geminiModelID) { newValue in
                                savePreferenceValue(
                                    newValue, forKey: "geminiModelID", defaultValue: GeminiClient.defaultModelID)
                                NotificationCenter.default.post(name: .checkGeminiStatus, object: nil)
                            }
                    }

                    Text(
                        "Your Gemini API key is used for screen text extraction.\nModel ID defaults to \(GeminiClient.defaultModelID) when left empty."
                    )
                    .font(.caption)
                    .foregroundColor(.secondary)
                }
...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, ObservableObject {
    ...
    private func setupMenu() {
        ...
        let openAIStatusItem = NSMenuItem(
            title: "OpenAI: Checking...", action: #selector(checkOpenAIStatus), keyEquivalent: "")
        openAIStatusItem.target = self
        openAIStatusItem.tag = MenuItemTag.openAIStatus
        menu.addItem(openAIStatusItem)

        let geminiStatusItem = NSMenuItem(
            title: "Gemini: Checking...", action: #selector(checkGeminiStatus), keyEquivalent: "")
    ...
    private func updateOpenAIStatusMenuItem() {
        guard let menu = statusItem.menu,
            let openAIStatusItem = menu.items.first(where: { $0.tag == MenuItemTag.openAIStatus })
        else {
            return
        }
    ...
    private func updateGeminiStatusMenuItem() {
        guard let menu = statusItem.menu,
            let geminiStatusItem = menu.items.first(where: { $0.tag == MenuItemTag.geminiStatus })
        else {
            return
        }
