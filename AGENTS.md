...
- Source code lives in `Sources/Rewrite` (SwiftUI/Cocoa app entry in `Rewrite.swift`).
- Build output goes to `.build/` (managed by SwiftPM).
- App bundle is assembled into `Rewrite.app/` by `build_app.sh`.
...
- `sh build_app.sh` — Release build via SwiftPM and creates `Rewrite.app` (codesigned locally).
- `swift build -c release` — Compile without bundling (binary in `.build/release/Rewrite`).
- `open Rewrite.app` — Launch the bundled app.
- `open Rewrite.app/Contents/MacOS/Rewrite` — Run from Terminal.
...
- File layout: one primary type per file; group related views/utilities near usage under `Sources/Rewrite`.
...
- No test target currently. If adding tests, use XCTest:
  - Create `Tests/RewriteTests` and mirror module structure.
  - Name tests `ThingTests.swift`, methods `test...`.
  - Run with `swift test` and keep fast, deterministic tests.

## Commit & Pull Request Guidelines
- Follow Conventional Commits seen in history: `feat: ...`, `fix: ...`, `docs: ...`.
...
- The app requires an OpenAI API key; it’s stored locally via app preferences/UserDefaults and only used for API calls.
- Do not hardcode keys. Users should set keys via Preferences.
- Note: App needs macOS Accessibility permission to read/replace selected text.
